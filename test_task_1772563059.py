...
class TestTaskManagement(unittest.TestCase):
    ...
    def test_add_task_basic(self):
        ...
        output = run_cli(["add", "测试任务_1772563059_基本添加"])
...
class TestExportFunctionality(unittest.TestCase):
    ...
    def test_export_csv(self):
        """测试导出为 CSV"""
        output = run_cli(
            ["export", "--format", "csv", "--output", "test_1772563059.csv"]
        )
        self.assertIn("✅ 已导出", output)
        self.assertTrue(os.path.exists("test_1772563059.csv"))

        with open("test_1772563059.csv", "r", encoding="utf-8-sig") as f:
            content = f.read()
            self.assertIn("ID", content)
            self.assertIn("内容", content)
            self.assertIn("状态", content)

        os.remove("test_1772563059.csv")

    def test_export_markdown(self):
        """测试导出为 Markdown"""
        output = run_cli(
            ["export", "--format", "markdown", "--output", "test_1772563059.md"]
        )
        self.assertIn("✅ 已导出", output)
        self.assertTrue(os.path.exists("test_1772563059.md"))

        with open("test_1772563059.md", "r", encoding="utf-8") as f:
            content = f.read()
            self.assertIn("# 任务列表", content)
            self.assertIn("| ID |", content)

        os.remove("test_1772563059.md")
